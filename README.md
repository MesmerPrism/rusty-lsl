...
LSLC-006B adds test-only deterministic conformance for the accepted UDP
discovery runtime: receive-order preservation, exact source/query identity,
consuming response allocation ownership, pre-cancellation precedence, and
caller-selected port cleanup. It changes no production behavior, discovery
policy, parsing, selection, activation, compatibility claim, device behavior,
