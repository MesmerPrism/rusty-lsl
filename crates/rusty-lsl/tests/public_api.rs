...
fn p4_session_batch_boundary_is_concrete_fail_closed_and_current_revision() {
    ...
    assert_eq!(rusty_lsl::ACCEPTED_FEATURE_LOCK_REVISION, 38);
    assert_eq!(rusty_lsl::runtime::ACCEPTED_FEATURE_LOCK_REVISION, 38);
}
...
fn p31_all_format_bounded_chunk_sessions_are_concrete_on_both_facades() {
    ...
    assert_eq!(rusty_lsl::ACCEPTED_FEATURE_LOCK_REVISION, 38);
    assert_eq!(runtime::ACCEPTED_FEATURE_LOCK_REVISION, 38);
}
...
const CONSUMER: &str = "rlsl-p6-host-qualification-v1";
...
fn identity(limits: StreamHandshakeLimits) -> StreamHandshakeIdentity {
    StreamHandshakeIdentity::new(
        ...
        "p6-loopback-host".into(),
        "p6-explicit-source".into(),
        "p6-host-session".into(),
        limits,
    )
    .unwrap()
}

struct HostClock([f64; 2], usize);
...
#[test]
fn p6_explicit_loopback_host_lifecycle_qualification() {
    let admission =
        admit_runtime_activation(ACCEPTED_FEATURE_LOCK_FINGERPRINT, CONSUMER, &selections())
            .unwrap();
    assert_eq!(
    ...
    // Explicit local discovery and receive-order selection (index zero).
    let responder = UdpSocket::bind("127.0.0.1:0").unwrap();
    let discovery_destination = responder.local_addr().unwrap();
    let (release_discovery, hold_discovery) = mpsc::channel();
    let body = "<?xml version=\"1.0\"?>\n<info>\n\
\t<name>p6-loopback</name>\n\
\t<type>qualification</type>\n\
\t<channel_count>1</channel_count>\n\
\t<channel_format>float32</channel_format>\n\
\t<source_id>p6-explicit-source</source_id>\n\
...
\t<desc />\n</info>\n";
    ...
    let query = ShortInfoQuery::new("name='p6-loopback'".into(), 16666, 66, query_limits).unwrap();
    ...
    assert_eq!(discovery.responses()[selected_index].query_id(), 66);
    assert_eq!(
        discovery.responses()[selected_index].source(),
        discovery_destination
    );

    // The selected host transaction continues through the existing two-record owner.
    let handshake_limits =
    ...
    let sent = TimestampedChunk::new(
        ...
        vec![
            make_sample(0x4092_5220_0000_0066, 0x3fa0_0066),
            make_sample(0x4092_5b80_0000_0067, 0xc020_0067),
        ],
    )
    .unwrap();
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    ...
    // A second admission is an explicit caller action, not ambient activation.
    let inlet_admission =
        admit_runtime_activation(ACCEPTED_FEATURE_LOCK_FINGERPRINT, CONSUMER, &selections())
            .unwrap();
    ...
    assert_eq!(
        received.samples()[1]
            ...
            .value()
            .to_bits(),
        0x4092_5b80_0000_0067
    );
    TcpListener::bind(session_address).unwrap();

    // Feed one exact received record through caller-bounded recovery, correction, and queue.
    let mut records = received.into_samples().into_iter();
    let first = records.next().unwrap();
    let correction_socket = UdpSocket::bind("127.0.0.1:0").unwrap();
    let correction_peer = correction_socket.local_addr().unwrap();
    ...
    let corrected = queue
        ...
        .unwrap();
    assert_eq!(corrected.sample().values()[0].to_bits(), 0x3fa0_0066);
